...
const char * const BITCOIN_CONF_FILENAME = "b3chain.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //b3chain-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Local\B3Chain
    // macOS: ~/Library/Application Support/B3Chain
    // Unix-like: ~/.b3chain
#ifdef WIN32
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "B3Chain";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/B3Chain";
#else
    // Unix-like
    return pathRet / ".b3chain";
#endif
#endif
}
