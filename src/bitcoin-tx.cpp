...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " b3chain-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The b3chain-tx tool is used for creating and modifying b3chain transactions.\n\n"
                "b3chain-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded b3chain transaction, or with \"-create [commands]\" to create a hex-encoded b3chain transaction.\n"
                "\n"
                "Usage: b3chain-tx [options] <hex-tx> [commands]\n"
                "or:    b3chain-tx [options] -create [commands]\n"
                "\n";
