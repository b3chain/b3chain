...
Options:
  -m, --multiprocess     Run multiprocess binaries b3chain-node, b3chain-gui.
  -M, --monolithic       Run monolithic binaries b3chaind, b3chain-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'b3chain-qt [ARGS]' or 'b3chain-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'b3chaind [ARGS]' or 'b3chain-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'b3chain-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'b3chain-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'b3chain-tx [ARGS]'.
...
Additional less commonly used commands:
  ...
  chainstate [ARGS] Run b3chain kernel chainstate util, equivalent to running 'b3chain-chainstate [ARGS]'.
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "b3chain-gui" : "b3chain-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "b3chain-node" : "b3chaind");
        } else if (cmd.command == "rpc") {
            args.emplace_back("b3chain-cli");
            // Since "b3chain rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("b3chain-util");
        } else {
...
//! Execute the specified b3chaind, b3chain-qt or other command line in `args`
...
//!             be a relative path to a b3chaind, b3chain-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/b3chain ...
