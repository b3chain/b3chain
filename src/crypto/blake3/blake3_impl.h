...
// b3chain: Detect CPU architecture for SIMD-accelerated BLAKE3.
// On x86/x86_64, the dispatch layer uses CPUID to select the fastest
// available implementation at runtime (SSE2 → SSE4.1 → AVX2 → AVX-512).
...
// SIMD implementation declarations — resolved at runtime by blake3_dispatch.c
#if defined(IS_X86)
...
void blake3_xof_many_avx512(const uint32_t cv[8],
                             const uint8_t block[BLAKE3_BLOCK_LEN],
                             uint8_t block_len, uint64_t counter,
                             uint8_t flags, uint8_t *out, size_t outblocks);
#endif
#endif
#endif
