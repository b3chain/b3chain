...
const QString BITCOIN_IPC_PREFIX("b3chain:");
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("b3chain://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'b3chain://' is not a valid URI. Use 'b3chain:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // b3chain: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid B3Chain address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
