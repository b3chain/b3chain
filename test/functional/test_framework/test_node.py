...
"""Class for b3chaind node under test"""
...
B3CHAIN_PID_FILENAME_DEFAULT = "b3chaind.pid"
...
class TestNode():
    """A class for representing a b3chaind node under test.
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, binaries, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, v2transport=False, uses_wallet=False, ipcbind=False):
        ...
        self.bitcoinconf = self.datadir_path / "b3chain.conf"
        ...
        # Configuration for logging is set as command-line args rather than in the b3chain.conf file.
        # This means that starting a b3chaind using the temp dir to debug a failed test won't
        # spam debug.log.
    ...
    def __del__(self):
        # Ensure that we don't leave any b3chaind processes lying around after
        # the test ends
        if self.process:
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # If listening and no -bind is given, then b3chaind would bind P2P ports on
        ...
        # b3chain.conf. To avoid collisions, change it to 127.0.0.1:tor_port().
        ...
        # Add a new stdout and stderr file each time b3chaind is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by b3chaind, and
        ...
        self.running = True
        self.log.debug("b3chaind started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self, *, wait_for_import=True):
        """Sets up an RPC connection to the b3chaind process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'b3chaind exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
                ...
                # Suppress if cookie file isn't generated yet and no rpcuser or rpcpassword; b3chaind may be starting.
                if "No RPC credentials" not in str(e):
                    raise
                latest_error = suppress_error("missing_credentials", e)
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error(f"Unable to connect to b3chaind after {self.rpc_timeout}s (ignored errors: {dict(suppressed_errors)!s}{'' if latest_error is None else f', latest: {latest_error[0]!r}/{latest_error[1]}'})")
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; b3chaind is still starting
    ...
    def wait_until_stopped(self, *, timeout=B3CHAIND_PROC_WAIT_TIMEOUT, expect_error=False, **kwargs):
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into b3chaind")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to b3chaind
        expected_msg: regex that stderr should match when b3chaind fails

        Will throw if b3chaind starts without an error.
        Will throw if an expected_msg is provided and it does not match b3chaind's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'b3chaind exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'b3chaind should have exited within {self.rpc_timeout}s '
...
class TestNodeCLI():
    """Interface to b3chain-cli for an individual node"""
    def __init__(self, binaries, datadir):
        self.options = []
        self.binaries = binaries
        self.datadir = datadir
        self.input = None
        self.log = logging.getLogger('TestFramework.b3chaincli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with b3chain-cli command-line options
    ...
    def send_cli(self, clicommand=None, *args, **kwargs):
        """Run b3chain-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running b3chain-cli {}".format(p_args[2:]))
