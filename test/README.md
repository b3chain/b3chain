This directory contains integration tests that test b3chaind and its
...
b3chaind and b3chain-qt by interacting with them through the RPC and P2P
interfaces.
...
The P2P and RPC ports used by the b3chaind nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another b3chaind
process running on the system (perhaps from a previous test which hasn't successfully
killed all its b3chaind nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other b3chaind processes are running.

On linux, the test framework will warn if there is another
b3chaind process running when the tests are started.

If there are zombie b3chaind processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
b3chaind processes running on the system, so should not be used if any non-test
b3chaind processes are being run.**
...
sure b3chaind processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and b3chaind `debug.log`s will all be dumped
...
`test_framework.log` and b3chaind `debug.log`s can be combined into a single
...
call methods that interact with the b3chaind nodes-under-test.

If further introspection of the b3chaind instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/b3chaind.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/b3chaind <pid>
```
