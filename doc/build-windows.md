...
Below are some notes on how to build B3Chain Core for Windows.

The options known to work for building B3Chain Core on Windows are:
...
Note that for WSL the B3Chain Core source path MUST be somewhere in the default mount file system, for
example /usr/src/b3chain, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\b3chain`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/b3chain
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/b3chain --strip
```
