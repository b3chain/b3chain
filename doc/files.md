...
The data directory is the default location where the B3Chain Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.b3chain/`
macOS    | `$HOME/Library/Application Support/B3Chain/`
Windows  | `%LOCALAPPDATA%\B3Chain\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `b3chain.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual B3Chain blocks (dumped in network format, 128 MiB per file)
...
`./`               | `b3chain.conf`        | User-defined [configuration settings](b3chain-conf.md) for `b3chaind` or `b3chain-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `b3chaind.pid`        | Stores the process ID (PID) of `b3chaind` or `b3chain-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `b3chaind` or `b3chain-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [b3chain.conf](b3chain-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`b3chain-qt` uses [`QSettings`](https://doc.qt.io/qt-6/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-6/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by B3Chain Core:
...
This table describes the files installed by B3Chain Core across different platforms.

| **Path**                                                   | **Description**                                                             |
|------------------------------------------------------------|-----------------------------------------------------------------------------|
| [README.md](README.md) or [readme.txt](README_windows.txt) | Project information and instructions                                        |
| b3chain.conf                                               | [Generated](../contrib/devtools/gen-bitcoin-conf.sh) configuration file     |
...
| libexec/bitcoin-gui                                        | IPC-enabled alternative to `b3chain-qt`                                     |
| libexec/b3chain-node                                       | IPC-enabled alternative to `b3chaind`                                       |
| libexec/test_b3chain                                       | Unit test binary                                                            |
| *libexec/test_b3chain-qt*                                  | GUI-specific unit tests                                                     |
| share/man/man1/                                            | Man pages for command-line tools like `b3chain-cli`, `b3chaind`, and others |
...
- README and b3chain.conf files are included in binary releases but not installed in source builds.
- On Windows, binaries have a `.exe` suffix (e.g., `b3chain-cli.exe`).
...
There have been multiple reports of database corruption and data loss when using this filesystem with B3Chain Core, see [Issue #31454](https://github.com/bitcoin/bitcoin/issues/31454) for more details.
