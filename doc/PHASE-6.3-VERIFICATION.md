...
|-------|------|----------------------|----------------------|----------|--------|
| S-1a  | `phase-a-codec` | SV2 type encoders + frame codec round-trip cleanly with golden vectors | `cd contrib/testnet/pool && node --env-file=.env.test --test --import tsx tests/sv2-codec.test.ts 2>&1 \| grep -c '# pass'` | ≥ 13 | `[-]` |
| S-1b  | `phase-a-noise` | Noise NX handshake + transport AEAD round-trip; tampered ciphertext rejected; 106-byte SignedCertificate signs and verifies | `cd contrib/testnet/pool && node --env-file=.env.test --test --import tsx tests/sv2-noise.test.ts 2>&1 \| grep -c '# pass'` | ≥ 6 | `[-]` |
...
| S-2b  | `phase-b-channel` | Channel layer pins extranonce_prefix per channel and reconstructs the full 8-byte extranonce slot at submit time | `cd contrib/testnet/pool && node --env-file=.env.test --test --import tsx tests/sv2-mining.test.ts 2>&1 \| grep -c '# pass'` | ≥ 5 | `[-]` |
...
| S-3a  | `phase-c-tp-msgs` | Template Distribution messages (NewTemplate / SetNewPrevHashTP / RequestTransactionData / SubmitSolutionTP / CoinbaseOutputDataSize) round-trip | `cd contrib/testnet/pool && node --env-file=.env.test --test --import tsx tests/sv2-tp.test.ts 2>&1 \| grep -c '# pass'` | ≥ 5 | `[-]` |
...
| S-4a  | `phase-d-jd-tokens` | JD token store issues 32-byte single-use tokens with TTL and rejects re-use / expired tokens | `cd contrib/testnet/pool && node --env-file=.env.test --test --import tsx tests/sv2-jd.test.ts 2>&1 \| grep -c '# pass'` | ≥ 4 | `[-]` |
...
| S-5a  | `phase-e-translator` | V1 mining.subscribe -> SV2 OpenExtendedMiningChannel -> V1 mining.notify roundtrip works against an in-process fake pool | `cd contrib/testnet/pool && node --env-file=.env.test --test --import tsx tests/sv2-translator.test.ts 2>&1 \| grep -c '# pass'` | ≥ 1 | `[-]` |
