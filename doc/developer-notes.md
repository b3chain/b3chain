...
configuration options and toggled while b3chaind is running with the `logging`
RPC.  For instance, launching b3chaind with `-debug` or `-debug=1` will turn on
...
You can then use the `-debug=lock` configuration option at b3chaind startup or
`b3chain-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `b3chain-cli logging [] '["lock"]'`.
...
$ valgrind -v --leak-check=full build/bin/b3chaind -printtoconsole
...
llvm-cov show \
    ...
    --project-title="B3Chain Core Coverage Report"
```
...
llvm-cov show \
    ...
    --show-expansions \
    --output-dir=build/coverage_report \
    --project-title="B3Chain Core Fuzz Coverage Report"
```
...
To profile a running b3chaind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep b3chaind` -- sleep 60
```
...
- [Main thread (`b3chaind`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `b3chaind.cpp`. Responsible for starting up and
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `b3chain-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `b3chain-cli` and the GUI debug console use this table to determine how to
