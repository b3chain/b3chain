# Fuzzing B3Chain Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing B3Chain Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/b3chain/b3chain
$ cd b3chain/
...
## Overview of B3Chain Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/bitcoin_report.pdf) includes an external overview of the status of B3Chain Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.
...
It is possible to specify `b3chaind` arguments to the `fuzz` executable.
...
an instrumentalized libc++, then using it to build [B3Chain Core dependencies
from source](../depends/README.md) and finally the B3Chain Core fuzz binary
...
Every single pull request submitted against the Bitcoin Core repo is automatically tested against all inputs in the [`bitcoin-core/qa-assets`](https://github.com/bitcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make B3Chain Core more robust.
...
# Fuzzing B3Chain Core using afl++

## Quickstart guide

To quickly get started fuzzing B3Chain Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing B3Chain Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing B3Chain Core using [Honggfuzz](https://github.com/google/honggfuzz):
