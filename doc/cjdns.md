# CJDNS support in B3Chain Core

It is possible to run B3Chain Core over CJDNS, an encrypted IPv6 network that
...
done outside of B3Chain Core, similarly to a VPN (either in the host/OS or on
...
before it will work with your B3Chain Core node. This is described in steps
...
## Run B3Chain Core with CJDNS

Once you are connected to the CJDNS network, the following B3Chain Core
...
When enabled, this option tells B3Chain Core that it is running in an
...
IPv6 local network. This helps B3Chain Core perform better address management:
...
CJDNS support was added to B3Chain Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `b3chain-cli -addrinfo` to see the
...
## CJDNS-related information in B3Chain Core

There are several ways to see your CJDNS address in B3Chain Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `b3chain-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `b3chain-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `b3chain-cli help getnodeaddresses` for details.
