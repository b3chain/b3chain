#!/usr/bin/env bash
# End-to-end rebrand deploy on seed1: patches + frontend rebuild + rsync.
set -euo pipefail
export LC_ALL=C
...
CONTRIB_LOGO="$CONTRIB/assets/b3chain-explorer-ng-logo.svg"
[ -f "$EXPLORER_NG_SRC/frontend/src/resources/b3chain-explorer-ng-logo.svg" ] || \
    cp "$CONTRIB_LOGO" "$EXPLORER_NG_SRC/frontend/src/resources/b3chain-explorer-ng-logo.svg"

echo "==> frontend rebuild (NODE_OPTIONS capped for 4GB VPS)"
