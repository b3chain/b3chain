...
#   3. RPC tip (from b3chaind) == backend's exposed tip on /api/v1/blocks/tip/height.
...
HTTP_TIP=$(curl -fsS "$EXPLORER_HOST/api/v1/blocks/tip/height" 2>/dev/null || echo "")
...
echo "==> 5. /resources/config.js served"
CFG_CT=$(curl -sI "$EXPLORER_HOST/resources/config.js" 2>/dev/null \
    | awk -F': ' 'tolower($1)=="content-type"{print tolower($2)}' | tr -d '\r')
CFG_CODE=$(curl -s -o /dev/null -w '%{http_code}' "$EXPLORER_HOST/resources/config.js" 2>/dev/null || echo "")
if [ "$CFG_CODE" = "200" ] && echo "$CFG_CT" | grep -q 'javascript'; then
    ok "config.js: $CFG_CODE $CFG_CT"
else
    bad "config.js: http=$CFG_CODE content-type=${CFG_CT:-missing}"
fi

echo "==> 6. websocket upgrade"
