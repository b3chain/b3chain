...
The bpftrace examples contain a relative path to the `b3chaind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `b3chaind` binary. The paths in the examples can be changed, for
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof b3chaind)
```
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof b3chaind)
```
...
```
$ ./build/bin/b3chaind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof b3chaind)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof b3chaind)
```
